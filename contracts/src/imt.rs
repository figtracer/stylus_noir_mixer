...
impl IncrementalMerkleTree {
	...
	pub fn insert(&mut self, leaf: FixedBytes<32>) -> Result<U32, ContractErrors> {
		...
		for i in 0..depth_u32 {
			if current_index % 2 == 0 {
				...
				self.cached_subtrees.insert(U32::from(i as u32), current_hash);
			} else {
		...
		let cur_root_idx: u32 = u32::from_be_bytes(self.current_root_index.get().to_be_bytes::<4>());
	...
	pub fn is_known_root(&self, root: FixedBytes<32>) -> bool {
		...
		let current_root_index_u32: u32 = u32::from_be_bytes(self.current_root_index.get().to_be_bytes::<4>());
